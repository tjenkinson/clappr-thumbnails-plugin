...
/***/ function(module, exports, __webpack_require__) {
	...
	},{key:'_updateCarouselPosition',value:function _updateCarouselPosition(){var hoverPosition=this._hoverPosition; // the time into the video at the current hover position
	var hoverTime=this.core.mediaControl.container.getDuration() * hoverPosition;var backdropWidth=this._$backdrop.width();var carouselWidth=this._$backdropCarousel.width(); // slide the carousel so that the image on the carousel that is above where the person
	...
	var thumbIndex=this._getThumbIndexForTime(hoverTime);var thumb=thumbs[thumbIndex];var thumbDuration=this._thumbDurations[thumbIndex]; // determine how far accross that thumbnail we are
	var timeIntoThumb=Math.max(hoverTime - thumb.time,0);var positionInThumb=thumbDuration !== Infinity?timeIntoThumb / thumbDuration:0;var xCoordInThumb=thumbWidth * positionInThumb; // now calculate the position along carousel that we want to be above the hover position
	...
	return 0;}},{key:'_renderPlugin',value:function _renderPlugin(){if(!this._loaded){return;}if(this._show && this._getOptions().thumbs.length > 0){this.$el.removeClass("hidden");this._updateCarouselPosition();}else {this.$el.addClass("hidden");}}},{key:'render',value:function render(){this._$backdrop = (0,_jQuery2['default'])("<div />").addClass("backdrop");this._$backdrop.height(this._getOptions().thumbHeight);this._$backdropCarousel = (0,_jQuery2['default'])("<div />").addClass("carousel");this._$backdrop.append(this._$backdropCarousel);(0,_jQuery2['default'])(this.el).append(this._$backdrop);this.$el.addClass("hidden");this._appendElToMediaControl();return this;}}]);return ScrubThumbnailsPlugin;})(_clappr.UICorePlugin);exports['default'] = ScrubThumbnailsPlugin;module.exports = exports['default'];
