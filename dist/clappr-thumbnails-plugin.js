...
/***/ function(module, exports, __webpack_require__) {
	...
	'use strict';Object.defineProperty(exports,'__esModule',{value:true});var _createClass=(function(){function defineProperties(target,props){for(var i=0;i < props.length;i++) {var descriptor=props[i];descriptor.enumerable = descriptor.enumerable || false;descriptor.configurable = true;if('value' in descriptor)descriptor.writable = true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};})();var _get=function get(_x,_x2,_x3){var _again=true;_function: while(_again) {var object=_x,property=_x2,receiver=_x3;desc = parent = getter = undefined;_again = false;if(object === null)object = Function.prototype;var desc=Object.getOwnPropertyDescriptor(object,property);if(desc === undefined){var parent=Object.getPrototypeOf(object);if(parent === null){return undefined;}else {_x = parent;_x2 = property;_x3 = receiver;_again = true;continue _function;}}else if('value' in desc){return desc.value;}else {var getter=desc.get;if(getter === undefined){return undefined;}return getter.call(receiver);}}};function _interopRequireDefault(obj){return obj && obj.__esModule?obj:{'default':obj};}function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError('Cannot call a class as a function');}}function _inherits(subClass,superClass){if(typeof superClass !== 'function' && superClass !== null){throw new TypeError('Super expression must either be null or a function, not ' + typeof superClass);}subClass.prototype = Object.create(superClass && superClass.prototype,{constructor:{value:subClass,enumerable:false,writable:true,configurable:true}});if(superClass)Object.setPrototypeOf?Object.setPrototypeOf(subClass,superClass):subClass.__proto__ = superClass;}var _clappr=__webpack_require__(1);var _jQuery=__webpack_require__(2);var _jQuery2=_interopRequireDefault(_jQuery);__webpack_require__(3);var ScrubThumbnailsPlugin=(function(_UICorePlugin){_inherits(ScrubThumbnailsPlugin,_UICorePlugin);_createClass(ScrubThumbnailsPlugin,[{key:'name',get:function get(){return 'scrub-thumbnails';}},{key:'attributes',get:function get(){return {'class':this.name};}}]);function ScrubThumbnailsPlugin(core){_classCallCheck(this,ScrubThumbnailsPlugin);_get(Object.getPrototypeOf(ScrubThumbnailsPlugin.prototype),'constructor',this).call(this,core);this._loaded = false;this._show = false; // proportion into seek bar that the user is hovered over 0-1
	this._hoverPosition = 0; // references to each thumbnail img
	this._$thumbs = []; // the duration of each thumbnail (ie time between it's start time and the start time of the next one)
	this._thumbDurations = [];this._activeThumbIndex = null;this._previousThumbIndex = null;this._nextThumbIndex = null;this._init();}_createClass(ScrubThumbnailsPlugin,[{key:'bindEvents',value:function bindEvents(){this.listenTo(this.core.mediaControl,_clappr.Events.MEDIACONTROL_MOUSEMOVE_SEEKBAR,this._onMouseMove);this.listenTo(this.core.mediaControl,_clappr.Events.MEDIACONTROL_MOUSELEAVE_SEEKBAR,this._onMouseLeave);}},{key:'_init',value:function _init(){var _this=this;this._calculateThumbDurations(); // preload all the thumbnails in the browser
	this._loadThumbnails(function(){ // all thumbnails now preloaded
	_this._loadBackdrop();_this._loaded = true;_this._renderPlugin();});}},{key:'_getOptions',value:function _getOptions(){if(!("scrubThumbnails" in this.core.options)){throw "'scrubThumbnails property missing from options object.";}return this.core.options.scrubThumbnails;}},{key:'_appendElToMediaControl',value:function _appendElToMediaControl(){this.core.mediaControl.$el.append(this.el);}},{key:'_onMouseMove',value:function _onMouseMove(e){this._calculateHoverPosition(event);this._show = true;this._renderPlugin();}},{key:'_onMouseLeave',value:function _onMouseLeave(){this._show = false;this._renderPlugin();}},{key:'_calculateHoverPosition',value:function _calculateHoverPosition(e){var offset=e.pageX - this.core.mediaControl.$seekBarContainer.offset().left; // proportion into the seek bar that the mouse is hovered over 0-1
	this._hoverPosition = Math.min(1,Math.max(offset / this.core.mediaControl.$seekBarContainer.width(),0));}},{key:'_calculateThumbDurations',value:function _calculateThumbDurations(){var thumbs=this._getOptions().thumbs;for(var i=0;i < thumbs.length;i++) {var current=thumbs[i];var next=i < thumbs.length - 1?thumbs[i + 1]:null;var duration=next?next.time - current.time:null;this._thumbDurations.push(duration);}} // download all the thumbnails
	},{key:'_loadThumbnails',value:function _loadThumbnails(onLoaded){var thumbs=this._getOptions().thumbs;var thumbsToLoad=thumbs.length;if(thumbsToLoad === 0){onLoaded();return;}var _iteratorNormalCompletion=true;var _didIteratorError=false;var _iteratorError=undefined;try{for(var _iterator=thumbs[Symbol.iterator](),_step;!(_iteratorNormalCompletion = (_step = _iterator.next()).done);_iteratorNormalCompletion = true) {var thumb=_step.value; // preload each thumbnail
	var img=new Image();img.src = thumb.url;img.onload = function(){if(--thumbsToLoad === 0){onLoaded();}};}}catch(err) {_didIteratorError = true;_iteratorError = err;}finally {try{if(!_iteratorNormalCompletion && _iterator['return']){_iterator['return']();}}finally {if(_didIteratorError){throw _iteratorError;}}}}},{key:'_loadBackdrop',value:function _loadBackdrop(){ // append each of the thumbnails to the backdrop
	var thumbs=this._getOptions().thumbs;var _iteratorNormalCompletion2=true;var _didIteratorError2=false;var _iteratorError2=undefined;try{for(var _iterator2=thumbs[Symbol.iterator](),_step2;!(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done);_iteratorNormalCompletion2 = true) {var thumb=_step2.value;var $img=(0,_jQuery2['default'])("<img />").attr("src",thumb.url).height(this._getOptions().thumbHeight);this._$backdropCarousel.append($img);this._$thumbs.push($img);}}catch(err) {_didIteratorError2 = true;_iteratorError2 = err;}finally {try{if(!_iteratorNormalCompletion2 && _iterator2['return']){_iterator2['return']();}}finally {if(_didIteratorError2){throw _iteratorError2;}}}} // calculate how far along the carousel should currently be slid
	// depending on where the user is hovering on the progress bar
	},{key:'_updateCarouselPosition',value:function _updateCarouselPosition(){var hoverPosition=this._hoverPosition;var videoDuration=this.core.mediaControl.container.getDuration(); // the time into the video at the current hover position
	var hoverTime=videoDuration * hoverPosition;var backdropWidth=this._$backdrop.width();var carouselWidth=this._$backdropCarousel.width(); // slide the carousel so that the image on the carousel that is above where the person
	// is hovering maps to that position in time.
	// Thumbnails may not be distributed at even times along the video
	var thumbs=this._getOptions().thumbs; // assuming that each thumbnail has the same width
	var thumbWidth=carouselWidth / thumbs.length; // determine which thumbnail applies to the current time
	var thumbIndex=this._getThumbIndexForTime(hoverTime);var thumb=thumbs[thumbIndex];var thumbDuration=this._thumbDurations[thumbIndex];if(thumbDuration === null){ // the last thumbnail duration will be null as it can't be determined
	...
	thumbDuration = Math.max(videoDuration - thumb.time,0);} // determine how far accross that thumbnail we are
	...
	var carouselXCoord=xCoordInCarousel - hoverPosition * backdropWidth;this._$backdropCarousel.css("left",-carouselXCoord);this._updateActiveThumb(hoverTime);} // marks the active thumbnail
	},{key:'_updateActiveThumb',value:function _updateActiveThumb(hoverTime){var thumbs=this._getOptions().thumbs;var thumbIndex=this._getThumbIndexForTime(hoverTime);var previousThumbIndex=thumbIndex > 0?thumbIndex - 1:null;var nextThumbIndex=thumbs.length > thumbIndex - 1?thumbIndex + 1:null;if(this._activeThumbIndex !== thumbIndex){if(this._activeThumbIndex !== null){this._$thumbs[this._activeThumbIndex].removeClass("active");}this._activeThumbIndex = thumbIndex;this._$thumbs[thumbIndex].addClass("active");}if(this._previousThumbIndex !== previousThumbIndex){if(this._previousThumbIndex !== null){this._$thumbs[this._previousThumbIndex].removeClass("next");}this._previousThumbIndex = previousThumbIndex;if(previousThumbIndex !== null){this._$thumbs[previousThumbIndex].addClass("next");}}if(this._nextThumbIndex !== nextThumbIndex){if(this._nextThumbIndex !== null){this._$thumbs[this._nextThumbIndex].removeClass("next");}this._nextThumbIndex = nextThumbIndex;if(nextThumbIndex !== null){this._$thumbs[nextThumbIndex].addClass("next");}}}},{key:'_updateSpotlightThumb',value:function _updateSpotlightThumb(){var hoverPosition=this._hoverPosition;var videoDuration=this.core.mediaControl.container.getDuration(); // the time into the video at the current hover position
	var hoverTime=videoDuration * hoverPosition; // determine which thumbnail applies to the current time
	var thumbIndex=this._getThumbIndexForTime(hoverTime);var thumb=thumbs[thumbIndex]; // update thumbnail
	this._$spotlightImg.attr("src",thumb.url);var elWidth=this.$el.width();var thumbWidth=this._$spotlightImg.width();var spotlightXPos=elWidth * hoverPosition - thumbWidth / 2; // adjust so the entire thumbnail is always visible
	spotlightXPos = Math.max(Math.min(spotlightXPos,elWidth - thumbWidth),0);this._$spotlight.css("left",spotlightXPos);} // returns the thumbnail which represents a time in the video
	// or null if there is no thumbnail that can represent the time
	},{key:'_getThumbIndexForTime',value:function _getThumbIndexForTime(time){for(var i=thumbs.length - 1;i >= 0;i--) {var thumb=thumbs[i];if(thumb.time <= time){return i;}} // stretch the first thumbnail back to the start
	return 0;}},{key:'_renderPlugin',value:function _renderPlugin(){if(!this._loaded){return;}if(this._show && this._getOptions().thumbs.length > 0){this.$el.removeClass("hidden");this._updateCarouselPosition();this._updateSpotlightThumb();}else {this.$el.addClass("hidden");}}},{key:'render',value:function render(){var thumbHeight=this._getOptions().thumbHeight;this._$backdrop = (0,_jQuery2['default'])("<div />").addClass("backdrop");this._$backdrop.height(thumbHeight);this._$backdropCarousel = (0,_jQuery2['default'])("<div />").addClass("carousel");this._$backdrop.append(this._$backdropCarousel);(0,_jQuery2['default'])(this.el).append(this._$backdrop);this._$spotlight = (0,_jQuery2['default'])("<div />").addClass("spotlight");this._$spotlight.height(thumbHeight);this._$spotlightImg = (0,_jQuery2['default'])("<img />");this._$spotlight.append(this._$spotlightImg);(0,_jQuery2['default'])(this.el).append(this._$spotlight);this.$el.addClass("hidden");this._appendElToMediaControl();return this;}}]);return ScrubThumbnailsPlugin;})(_clappr.UICorePlugin);exports['default'] = ScrubThumbnailsPlugin;module.exports = exports['default'];
