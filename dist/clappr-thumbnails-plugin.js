...
/***/ function(module, exports, __webpack_require__) {
	...
	'use strict';Object.defineProperty(exports,"__esModule",{value:true});var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value" in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};}();var _clappr=__webpack_require__(1);var _jquery=__webpack_require__(2);var _jquery2=_interopRequireDefault(_jquery);__webpack_require__(4);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}function _possibleConstructorReturn(self,call){if(!self){throw new ReferenceError("this hasn't been initialised - super() hasn't been called");}return call&&(typeof call==="object"||typeof call==="function")?call:self;}function _inherits(subClass,superClass){if(typeof superClass!=="function"&&superClass!==null){throw new TypeError("Super expression must either be null or a function, not "+typeof superClass);}subClass.prototype=Object.create(superClass&&superClass.prototype,{constructor:{value:subClass,enumerable:false,writable:true,configurable:true}});if(superClass)Object.setPrototypeOf?Object.setPrototypeOf(subClass,superClass):subClass.__proto__=superClass;}var ScrubThumbnailsPlugin=function(_UICorePlugin){_inherits(ScrubThumbnailsPlugin,_UICorePlugin);_createClass(ScrubThumbnailsPlugin,[{key:'name',get:function get(){return 'scrub-thumbnails';}},{key:'attributes',get:function get(){return {'class':this.name};} /* 
	   ...
	   */}],[{key:'buildSpriteConfig',value:function buildSpriteConfig(spriteSheetUrl,numThumbs,thumbWidth,thumbHeight,numColumns,timeInterval,startTime){startTime=startTime||0;var thumbs=[];for(var i=0;i<numThumbs;i++){thumbs.push({url:spriteSheetUrl,time:startTime+i*timeInterval,w:thumbWidth,h:thumbHeight,x:i%numColumns*thumbWidth,y:Math.floor(i/numColumns)*thumbHeight});}return thumbs;}}]);function ScrubThumbnailsPlugin(core){_classCallCheck(this,ScrubThumbnailsPlugin);var _this=_possibleConstructorReturn(this,Object.getPrototypeOf(ScrubThumbnailsPlugin).call(this,core));_this._loaded=false;_this._show=false; // proportion into seek bar that the user is hovered over 0-1
	_this._hoverPosition=0; // each element is {x, y, w, h, imageW, imageH, url, time, duration}
	// one entry for each thumbnail
	_this._thumbs=[];_this._$backdropCarouselImgs=[];_this._init();return _this;}_createClass(ScrubThumbnailsPlugin,[{key:'bindEvents',value:function bindEvents(){this.listenTo(this.core.mediaControl,_clappr.Events.MEDIACONTROL_MOUSEMOVE_SEEKBAR,this._onMouseMove);this.listenTo(this.core.mediaControl,_clappr.Events.MEDIACONTROL_MOUSELEAVE_SEEKBAR,this._onMouseLeave);this.listenTo(this.core.mediaControl,_clappr.Events.MEDIACONTROL_RENDERED,this._onMediaControlRendered);}},{key:'_init',value:function _init(){var _this2=this; // preload all the thumbnails in the browser
	this._loadThumbnails(function(){ // all thumbnails now preloaded
	_this2._loadBackdrop();_this2._loaded=true;_this2._renderPlugin();});}},{key:'_getOptions',value:function _getOptions(){if(!("scrubThumbnails" in this.core.options)){throw "'scrubThumbnails property missing from options object.";}return this.core.options.scrubThumbnails;}},{key:'_appendElToMediaControl',value:function _appendElToMediaControl(){ // insert after the background
	this.core.mediaControl.$el.find(".media-control-background").first().after(this.el);}},{key:'_onMediaControlRendered',value:function _onMediaControlRendered(){this._appendElToMediaControl();}},{key:'_onMouseMove',value:function _onMouseMove(e){this._calculateHoverPosition(e);this._show=true;this._renderPlugin();}},{key:'_onMouseLeave',value:function _onMouseLeave(){this._show=false;this._renderPlugin();}},{key:'_calculateHoverPosition',value:function _calculateHoverPosition(e){var offset=e.pageX-this.core.mediaControl.$seekBarContainer.offset().left; // proportion into the seek bar that the mouse is hovered over 0-1
	this._hoverPosition=Math.min(1,Math.max(offset/this.core.mediaControl.$seekBarContainer.width(),0));} // download all the thumbnails
	},{key:'_loadThumbnails',value:function _loadThumbnails(onLoaded){var _this3=this;var thumbs=this._getOptions().thumbs;var cachedImageUrls=[];var thumbsToLoad=thumbs.length;if(thumbsToLoad===0){onLoaded();return;}var _loop=function _loop(i){var thumb=thumbs[i];_this3._thumbs.push(null);var next=i<thumbs.length-1?thumbs[i+1]:null; // the duration this thumb lasts for
	// if it is the last thumb then duration will be null
	var duration=next?next.time-thumb.time:null; // preload each thumbnail
	var $img=(0,_jquery2.default)("<img />");var onImgLoaded=function onImgLoaded(){ // put image in dom to prevent browser removing it from cache
	if(cachedImageUrls.indexOf(thumb.url)===-1){ // not been cached
	_this3._$imageCache.append($img);cachedImageUrls.push(thumb.url);}var imageW=$img[0].width;var imageH=$img[0].height;_this3._thumbs[i]={imageW:imageW, // actual width of image
	...
	url:thumb.url,time:thumb.time, // time this thumb represents
	duration:duration // how long (from time) this thumb represents
	};if(--thumbsToLoad===0){onLoaded();}};$img.one("load",onImgLoaded).attr("src",thumb.url);};for(var i=0;i<thumbs.length;i++){_loop(i);}} // builds a dom element which represents the thumbnail
	// scaled to the provided height
	},{key:'_buildImg',value:function _buildImg(thumb,height){var scaleFactor=height/thumb.h;var $img=(0,_jquery2.default)("<img />").addClass("thumbnail-img").attr("src",thumb.url); // the container will contain the image positioned so that the correct sprite
	// is visible
	var $container=(0,_jquery2.default)("<div />").addClass("thumbnail-container");$container.width(thumb.w*scaleFactor);$container.height(height);$img.css({height:thumb.imageH*scaleFactor,left:-1*thumb.x*scaleFactor,top:-1*thumb.y*scaleFactor});$container.append($img);return $container;}},{key:'_loadBackdrop',value:function _loadBackdrop(){if(!this._getOptions().backdropHeight){ // disabled
	return;} // append each of the thumbnails to the backdrop
	//for(let thumb of this._thumbs) {
	for(var i=0;i<this._thumbs.length;i++){var _thumb=this._thumbs[i];var _$img=this._buildImg(_thumb,this._getOptions().backdropHeight);this._$backdropCarousel.append(_$img);this._$backdropCarouselImgs.push(_$img);}} // calculate how far along the carousel should currently be slid
	...
	var carouselXCoord=xCoordInCarousel-hoverPosition*backdropWidth;this._$backdropCarousel.css("left",-carouselXCoord); // now update the transparencies so that they fade in around the active one
	...
	var opacity=Math.max(0.6-Math.abs(distance)/(2*thumbWidth),0.08);this._$backdropCarouselImgs[i].css("opacity",opacity);}}},{key:'_updateSpotlightThumb',value:function _updateSpotlightThumb(){if(!this._getOptions().spotlightHeight){ // disabled
	return;}var hoverPosition=this._hoverPosition;var videoDuration=this.core.mediaControl.container.getDuration(); // the time into the video at the current hover position
	...
	this._$spotlight.empty();this._$spotlight.append(this._buildImg(thumb,this._getOptions().spotlightHeight));var elWidth=this.$el.width();var thumbWidth=this._$spotlight.outerWidth();var spotlightXPos=elWidth*hoverPosition-thumbWidth/2; // adjust so the entire thumbnail is always visible
	...
	},{key:'_getThumbIndexForTime',value:function _getThumbIndexForTime(time){var thumbs=this._thumbs;for(var i=thumbs.length-1;i>=0;i--){var _thumb2=thumbs[i];if(_thumb2.time<=time){return i;}} // stretch the first thumbnail back to the start
	return 0;}},{key:'_renderPlugin',value:function _renderPlugin(){if(!this._loaded){return;}if(this._show&&this._getOptions().thumbs.length>0){this.$el.removeClass("hidden");this._updateCarousel();this._updateSpotlightThumb();}else {this.$el.addClass("hidden");}}},{key:'render',value:function render(){this._$imageCache=(0,_jquery2.default)("<div />").addClass("image-cache");(0,_jquery2.default)(this.el).append(this._$imageCache); // if either of the heights are null or 0 then that means that part is disabled
	if(this._getOptions().backdropHeight){this._$backdrop=(0,_jquery2.default)("<div />").addClass("backdrop");this._$backdrop.height(this._getOptions().backdropHeight);this._$backdropCarousel=(0,_jquery2.default)("<div />").addClass("carousel");this._$backdrop.append(this._$backdropCarousel);(0,_jquery2.default)(this.el).append(this._$backdrop);}var spotlightHeight=this._getOptions().spotlightHeight;if(spotlightHeight){this._$spotlight=(0,_jquery2.default)("<div />").addClass("spotlight");this._$spotlight.height(spotlightHeight);(0,_jquery2.default)(this.el).append(this._$spotlight);}this.$el.addClass("hidden");this._appendElToMediaControl();return this;}}]);return ScrubThumbnailsPlugin;}(_clappr.UICorePlugin);exports.default=ScrubThumbnailsPlugin;module.exports=exports['default'];
...
/***/ function(module, exports, __webpack_require__) {
	...
	if(false) {
		...
		if(!content.locals) {
			module.hot.accept("!!./../node_modules/css-loader/index.js!./../node_modules/sass-loader/index.js?includePaths[]=/home/heberte/terabit/clappr-thumbnails-plugin/node_modules/compass-mixins/lib!./style.sass", function() {
				var newContent = require("!!./../node_modules/css-loader/index.js!./../node_modules/sass-loader/index.js?includePaths[]=/home/heberte/terabit/clappr-thumbnails-plugin/node_modules/compass-mixins/lib!./style.sass");
